...
export class ChessBoard {
    ...
    private kingPositions: Record<Color, Position>;
    ...
    private findInitialKingPositions(): Record<Color, Position> {
        const kingPositions: Record<Color, Position> = { white: { row: -1, col: -1 }, black: { row: -1, col: -1 } };
        ...
        for (let row = 0; row < 8; row++) {
            for (let col = 0; col < 8; col++) {
                ...
                if (piece.color === "white") {
                    kingPositions.white = { row, col };
                } else {
                    kingPositions.black = { row, col };
                }
            }
        }
